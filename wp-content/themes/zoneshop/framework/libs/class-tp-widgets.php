...
abstract class Thim_Widget extends WP_Widget {
	...
	function render_field( $name, $field, $value, $repeater = array(), $is_template = false ) {
		...
		if ( ! empty( $field['state_name'] ) ) {
			$wrapper_classes[] = 'thim-widget-field-state-' . $field['state_name'];
		}
		if ( ! empty( $field['hidden'] ) ) {
			$wrapper_classes[] = 'thim-widget-field-is-hidden';
		}
		if ( ! empty( $field['optional'] ) ) {
			$wrapper_classes[] = 'thim-widget-field-is-optional';
		}

		?>
		<div class="<?php echo implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ?><?php if ( ! empty( $field['class'] ) ) {
			echo ' ' . $field['class'];
		} ?>"><?php
