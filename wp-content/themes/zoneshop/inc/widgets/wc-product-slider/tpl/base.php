...
//$show_image    = $instance['show_image'] ? 1 : 0;
$column_slider = $instance['column_slider'];
$cats          = $instance['cats'];
//$product_slider_show_image = $instance['product_slider_show_image'];
$show_rating = true;
//$show_button = $instance['show_button'] ? 1 : 0;
...
//}
if ( $column_slider ) {
	$data_show_button .= ' data-column-slider="' . $column_slider . '"';
}
...
if ( $r->have_posts() ) {
	...
	//var_dump($column_slider);
//	$column_imgaes = $column_slider - 1;
	...
	if ( $link_images <> '' ) {
		echo '<div class="product-images ' . $column . '"><div class="wrapper">' . $before_link . '<img src="' . $link_images . '" alt=""/>' . $after_link . '</div></div>';
	}
	echo '<div class="wrapper-product-slider ' . $col_box_slider . '">';
	echo '<div class="product-grid category-product-list box-content owl-carousel owl-theme slider-' . $column_slider . '-column" ' . $data_show_button . '>';
	$i = $j = 1;
	while ( $r->have_posts() ) :
		$r->the_post();
		if ( $i == 1 ) {
